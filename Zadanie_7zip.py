...
class TestPositive:
    def test_add_archive(self, make_folder, clear_folder, make_files):  # a создали архив
        res_add = checkout(f'cd {data["folder_in"]}; 7z -t  {data["folder_out"]}/arx2', "Everything is Ok")
    ...
    def test_check_e_extract(self, clear_folder, make_files):  #
        res = list()
        res.append(checkout(f'cd {data["folder_in"]}; 7z -t  {data["folder_out"]}/arx2', "Everything is Ok"))
    ...
    def test_update(self):  # u - обновление архива
     ...
     def test_check_archive(clear_folder, make_files):  #
         ...
         result_add = checkout(f'cd {folder_in}; 7z -t  {folder_out}/arx2', "Everything is Ok")
    ...
    def test_nonempty_archive(self, clear_folder, make_files):
     ...
     def test_check_list_archive(clear_folder, make_files):
        res = list()
         res.append(checkout(f'cd {folder_in}; 7z -t  {folder_out}/arx2', "Everything is Ok"))
